...
@blueprint.route('/eot_reports', methods=['GET'])
def eot_reports():
    """Fetches pupil marks per subject for a given assessment and renders the eot_reports page,
...
def delete_scores():
    ...
    if not score_ids:
        ...
        return redirect(url_for('eot_reports_blueprint.eot_reports'))
...
@blueprint.route('/term_eot_reports', methods=['GET'])
def term_eot_reports():
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    if not any(filters.values()):
        return render_template(
            'eot_reports/term_eot_reports.html',
            eot_reports=[],
            subject_names=[],
            class_list=class_list,
            study_years=study_years,
    ...
    eot_reports = list(pivoted.values())
...
@blueprint.route('/scores_eot_reports', methods=['GET'])
def scores_eot_reports():
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    if not any(filters.values()):
        return render_template('eot_reports/scores_eot_reports.html',
            eot_reports=[], subject_names=[], class_list=class_list,
            study_years=study_years, terms=terms, assessments=assessments,
            selected_class_id=None, selected_study_year_id=None,
            selected_term_id=None, selected_assessment_name=None, segment='eot_reports'
        )
    ...
    for student in student_map.values():
        ...
        eot_reports.append(student)

    return render_template('eot_reports/scores_eot_reports.html',
        eot_reports=eot_reports, subject_names=subject_names,
...
@blueprint.route('/scores_positions_eot_reports_remarks', methods=['GET'])
def scores_positions_eot_reports_remarks():  
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    if not all([class_id, year_id, term_id, assessment_name]):
        return render_template('eot_reports/scores_positions_eot_reports_remarks.html',
            eot_reports=[], subject_names=[], class_list=class_list,
            study_years=study_years, terms=terms, assessments=assessments,
            selected_class_id=class_id, selected_study_year_id=year_id,
            selected_term_id=term_id, selected_assessment_name=assessment_name,
...
def assessment_report():
    ...
    if not any(filters.values()):
        return render_template('eot_reports/assessment_report.html',
            eot_reports=[], pivoted_columns=[],
            class_list=class_list,
            study_years=study_years,
            terms=terms,
            assessments=assessments,
    ...
    # Sort and rank
    eot_reports.sort(key=lambda x: x['average_score'], reverse=True)
    current_rank = 1
    last_avg = None
    for index, student in enumerate(eot_reports):
...
def term_report_card(reg_no):
    ...
    return render_template("eot_reports/term_report_card.html",
        pupil=pupil,
...
@blueprint.route('/scores_p_eot_reports', methods=['GET'])
def scores_p_eot_reports():
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    # If no filters selected, render empty page
    if not any(filters.values()):
        return render_template('eot_reports/scores_p_eot_reports.html',
            eot_reports=[], subject_names=[], class_list=class_list,
            study_years=study_years, terms=terms, assessments=assessments,
            selected_class_id=None, selected_study_year_id=None,
            selected_term_id=None, selected_assessment_name=None, segment='eot_reports'
...
def vd_eot_reports():
    ...
    # --- Get class teacher and signature ---
    cursor.execute("""
    ...
    # --- Get headmaster signature image ---
    cursor.execute("""
        SELECT sign_image
        FROM users
    ...
    sql = f"""
        ...
        LEFT JOIN grades g ON s.Mark BETWEEN g.min_score AND g.max_score
        JOIN (
            SELECT class_id, COUNT(*) total_class_size
            FROM pupils GROUP BY class_id
        ) cc ON cc.class_id = p.class_id
        JOIN (
            SELECT class_id, stream_id, COUNT(*) total_stream_size
            FROM pupils GROUP BY class_id, stream_id
        ) sc ON sc.class_id = p.class_id AND sc.stream_id = p.stream_id
        WHERE p.class_id = %s
          AND s.year_id = %s
          AND s.term_id = %s
        ...
        if r['Mark'] is not None:
            cursor.execute("""
                ...
                WHERE sc.subject_id = %s
                  AND sc.stream_id = %s
                  AND %s BETWEEN sc.min_score AND sc.max_score
    ...
    for stu in grouped.values():
        ...
        cursor.execute("""
            ...
            WHERE cc.stream_id = %s
              AND %s BETWEEN cc.min_score AND cc.max_score
    ...
    # assign positions
    reports_list.sort(key=lambda x: (-x['average_score'], x['full_name']))
    for idx, stu in enumerate(reports_list, 1):
        stu['class_position'] = idx

    by_stream = defaultdict(list)
    for stu in reports_list:
        by_stream[(stu['assessment_name'], stu['stream_name'])].append(stu)
    for group in by_stream.values():
        group.sort(key=lambda x: -x['average_score'])
...
@blueprint.route('/vd_eot_reports_2', methods=['GET'])
def vd_eot_reports_2():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    ...
    if not all([class_id, stream_id, year_id, term_id, assessment_name_list]):
        ...
        conn.close()
        return render_template(
            'eot_reports/vd_eot_reports_2.html',
            eot_reports=[],
            subject_names=[],
            class_list=class_list,
            study_years=study_years,
    ...
    return render_template(
        'eot_reports/vd_eot_reports_2.html',
        eot_reports=list(class_grouped.values()),
        subject_names=sorted(subject_names),
        class_list=class_list,
        study_years=study_years,
        terms=terms,
...
@blueprint.route('/scores_positions_eot_reports_2', methods=['GET'])
def scores_positions_eot_reports_2():
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    if not all([class_id, year_id, term_id, assessment_name]):
        ...
        connection.close()
        return render_template(
            'eot_reports/scores_positions_eot_reports_2.html',
            eot_reports=[],
            subject_names=[],
            class_list=class_list,
            study_years=study_years,
...
@blueprint.route('/vd_eot_reports_3', methods=['GET'])
def vd_eot_reports_3():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    ...
    if not all([class_id, stream_id, year_id, term_id, assessment_name_list]):
        cursor.close()
        conn.close()
        return render_template('eot_reports/vd_eot_reports_3.html',
            eot_reports=[], subject_names=[],
            class_list=class_list, study_years=study_years,
...
@blueprint.route('/scores_positions_eot_reports_3', methods=['GET'])
def scores_positions_eot_reports_3():
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    if not all([class_id, year_id, term_id, assessment_name]):
        ...
        connection.close()
        return render_template(
            'eot_reports/scores_positions_eot_reports_3.html',
            eot_reports=[],
            subject_names=[],
            class_list=class_list,
            study_years=study_years,
