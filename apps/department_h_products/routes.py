...
from apps.department_h_products import blueprint
...
@blueprint.route('/dep_head_products')
def dep_head_products():
    """Renders products page showing only those in categories assigned to the logged-in user via category_roles."""
    try:
        ...
        # Load all categories assigned to this user
        cursor.execute('''
            SELECT DISTINCT cl.name
            FROM category_list cl
            INNER JOIN category_roles cr ON cl.CategoryID = cr.category_id
            WHERE cr.user_id = %s
            ORDER BY cl.name
        ''', (user_id,))
        categories = [row['name'] for row in cursor.fetchall()]

        # Load sub-categories within user's assigned categories
        cursor.execute('''
            SELECT DISTINCT
                sc.sub_category_id,
            ...
            INNER JOIN category_list cl ON sc.category_id = cl.CategoryID
            INNER JOIN category_roles cr ON cl.CategoryID = cr.category_id
            WHERE cr.user_id = %s
    ...
    return render_template(
        'department_h_products/products.html',
        products=products,
...
@blueprint.route('/cat_head_add_product', methods=['GET', 'POST'])
def cat_head_add_product():
    ...
    # Fetch categories assigned to this user via category_roles
    cursor.execute('''
    ...
    # Fetch sub-categories only for those categories
    cursor.execute('''
        SELECT sc.sub_category_id, sc.name, sc.category_id 
        FROM sub_category sc
        WHERE sc.category_id IN (
            SELECT category_id FROM category_roles WHERE user_id = %s
        )
        ORDER BY sc.name
    ''', (user_id,))
    sub_categories = cursor.fetchall()
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for('department_h_blueprint.dep_head_product'))
    ...
    return render_template(
        'department_h_products/add_product.html',
        categories=categories,
        sub_categories=sub_categories,
...
@blueprint.route('/cat_head_edit_product/<int:product_id>', methods=['GET', 'POST'])
def cat_head_edit_product(product_id):
    if 'id' not in session:
    ...
    if not product:
        ...
        return redirect(url_for('department_h_blueprint.dep_head_products'))

    # Check if user has access to this product's category
    cursor.execute('''
        SELECT 1 FROM category_roles
        WHERE user_id = %s AND category_id = %s
    ''', (user_id, product['category_id']))
    access = cursor.fetchone()

    if not access:
        flash("You do not have permission to edit this product.", "danger")
        return redirect(url_for('department_h_blueprintt.dep_head_products'))
    ...
    # Fetch sub-categories only within those categories
    cursor.execute('''
        SELECT sc.sub_category_id, sc.name, sc.category_id
        FROM sub_category sc
        WHERE sc.category_id IN (
    ...
    if request.method == 'POST':
        ...
        # Validate category permission again on submission
        cursor.execute('''
            SELECT 1 FROM category_roles
            WHERE user_id = %s AND category_id = %s
        ''', (user_id, category_id))
        ...
        if not access_check:
            flash("You do not have permission to assign this category.", "danger")
            return redirect(url_for('department_h_blueprint.dep_head_products'))

        # Handle image upload
        image_file = request.files.get('image')
        image_filename = product['image']  # Default to existing image
    ...
    return render_template('department_h_products/edit_product.html',
                           product=product,
                           categories=categories,
                           sub_categories=sub_categories)
...
@blueprint.route('/cat_head_subc_data', methods=['GET'])
def cat_head_subc_data():
    if 'id' not in session:
    ...
    # Fetch sub-categories for the given category_id only if the user has access
    cursor.execute('''
        SELECT sc.*
        FROM sub_category sc
        JOIN category_roles cr ON sc.category_id = cr.category_id
        WHERE sc.category_id = %s AND cr.user_id = %s
        ORDER BY sc.name
    ''', (category_id, user_id))
...
@blueprint.route('/cat_head_sub_category_data')
def cat_head_sub_category_data():
    if 'id' not in session:
    ...
    try:
        ...
        cursor.execute('''
            ...
            JOIN category_roles cr ON cr.category_id = c.CategoryID
            WHERE cr.user_id = %s
            ORDER BY sc.name
        ''', (user_id,))
